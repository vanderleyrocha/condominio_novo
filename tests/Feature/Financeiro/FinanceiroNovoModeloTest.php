...
use App\Actions\Financeiro\RegistrarPagamentoNovo;
...
it('registra pagamento com distribuição FIFO e recalcula status', function () {
    ...
    $pagamento = app(RegistrarPagamentoNovo::class)->executar(
...
it('rejeita pagamento de pessoa sem vínculo vigente com a unidade', function () {
    ...
    expect(fn () => app(RegistrarPagamentoNovo::class)->executar(
...
it('estorna com tetos e reabre a taxa; segundo estorno é bloqueado', function () {
    ...
    expect(fn () => app(EstornarPagamentoNovo::class)->executar($pagamento, [$taxa->id => 150.00]))
        ->toThrow(DomainException::class, 'maior que o valor pago');

    $estorno = app(EstornarPagamentoNovo::class)->executar($pagamento->fresh(), [$taxa->id => 100.00]);
    ...
    expect(fn () => app(EstornarPagamentoNovo::class)->executar($pagamento->fresh(), [$taxa->id => 10.00]))
...
it('grade anual gera pagamento real pelo delta e ajuste negativo na redução', function () {
    ...
    expect($taxa->fresh()->status)->toBe(StatusTaxa::Pago)
        ->and(PagamentoNovo::query()->count())->toBe(1);
    ...
    expect($taxa->fresh()->status)->toBe(StatusTaxa::PagoParcial)
        ...
        ->and((float) PagamentoNovo::query()->min('valor_total'))->toBe(-60.0);
...
it('aplica a matriz de papéis: estorno é admin-only; sindico gere taxas', function () {
    ...
    $pagamento = PagamentoNovo::factory()->create();
    ...
    expect($admin->can('estornar', $pagamento))->toBeTrue()
        ...
        ->and($proprietario->can('create', PagamentoNovo::class))->toBeFalse();
});
...
it('renderiza as telas novas do financeiro para admin', function () {
    ...
    foreach (['taxas.index', 'taxas.lancar', 'pagamentos-novo.index', 'pagamentos-novo.create',
        'lancamentos.index', 'inadimplencia.index', 'resumo-novo.index', 'resumo-novo.intervalo',
        'painel-novo', 'cobrancas-extraordinarias.index'] as $rota) {
