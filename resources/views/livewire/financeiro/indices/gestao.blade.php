<div class="space-y-6">
    <div class="card">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
            <h2 class="text-lg font-semibold text-slate-900">Índices Econômicos</h2>
            <div class="flex items-center gap-3">
                <select wire:model.live="tipoFiltro" class="input w-40">
                    @foreach ($tipos as $tipo)
                        <option value="{{ $tipo->value }}">{{ $tipo->rotulo() }}</option>
                    @endforeach
                </select>
                <button type="button" wire:click="novoIndice" class="btn btn-primary">Novo índice</button>
            </div>
        </div>
            ...
            <form wire:submit="salvar" class="mb-6 rounded-lg border border-slate-200 bg-slate-50/50 p-4 sm:p-5">
                ...
                <div class="grid gap-4 md:grid-cols-4">
                    <div>
                        ...
                        <select wire:model="formTipo" class="input">
                            @foreach ($tipos as $tipo)
                                <option value="{{ $tipo->value }}">{{ $tipo->rotulo() }}</option>
                            @endforeach
                        </select>
                        @error('formTipo') <p class="error-text text-xs">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="label">Ano</label>
                        <input type="number" wire:model="formAno" class="input">
                        @error('formAno') <p class="error-text text-xs">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="label">Mês</label>
                        <select wire:model="formMes" class="input">
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="formIndice" placeholder="0,0000" class="input text-right">
            ...
            <tbody>
                @forelse ($indices as $indice)
                    <tr wire:key="indice-{{ $indice->id }}">
                        ...
                        <td class="text-right">
                            <button type="button" wire:click="editar({{ $indice->id }})"
                                    class="btn btn-primary btn-sm">Editar</button>
                            <button type="button" wire:click="excluir({{ $indice->id }})"
