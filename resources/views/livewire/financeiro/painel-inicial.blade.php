<div class="space-y-6">
    ...
    <div class="grid gap-4 sm:gap-6 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="card">
            <div class="flex items-center justify-between gap-2">
                <p class="section-label">Mensalidades em aberto</p>
        ...
        <div class="card">
            ...
            @if ($ultimaReceita)
                ...
                <p class="mt-1.5 truncate text-xs text-slate-500">{{ $ultimaReceita->data->format('d/m/Y') }} — {{ $ultimaReceita->descricao }}</p>
            @else
        ...
        <div class="card">
            ...
            @if ($ultimaDespesa)
                ...
                <p class="mt-1.5 truncate text-xs text-slate-500">{{ $ultimaDespesa->data->format('d/m/Y') }} — {{ $ultimaDespesa->descricao }}</p>
            @else
                <p class="mt-3 text-sm text-slate-500">Nenhum registro encontrado.</p>
            @endif
        </div>
            ...
            <table class="table-modern">
                ...
                <tbody>
                    @forelse ($ultimosPagamentos as $pagamento)
                        <tr>
                            <td class="whitespace-nowrap">{{ $pagamento->data->format('d/m/Y') }}</td>
                            ...
                            <td class="whitespace-nowrap text-right tabular-nums">{{ \App\Support\DinheiroBr::formatar($pagamento->valor) }}</td>
                        </tr>
                    @empty
                        <tr>
