<div class="mx-auto max-w-3xl space-y-6">
        ...
        <div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
            ...
            @if ($pagamento->estornoDe !== null)
                <p class="sm:col-span-2 text-red-600">
                    ...
                    <a href="{{ route('pagamentos-novo.show', $pagamento->estornoDe) }}" class="underline">#{{ $pagamento->estornoDe->id }}</a>
                </p>
            @endif
            @if ($pagamento->estornos->isNotEmpty())
                <p class="sm:col-span-2 text-red-600">
                    ...
                    @foreach ($pagamento->estornos as $estorno)
                        <a href="{{ route('pagamentos-novo.show', $estorno) }}" class="underline">estorno #{{ $estorno->id }}</a>
                    @endforeach
                </p>
            @endif
        </div>
    ...
    <div class="flex items-center gap-3">
        @can('emitirRecibo', $pagamento)
            <a href="{{ route('pdf-novo.pagamentos.recibo', $pagamento) }}" target="_blank" class="btn btn-secondary">Recibo</a>
        @endcan
        @can('estornar', $pagamento)
            @if (! $pagamento->isEstorno() && $pagamento->estornos->isEmpty())
                <a href="{{ route('pagamentos-novo.estorno', $pagamento) }}" class="btn btn-danger">Estornar</a>
            @endif
        @endcan
        <a href="{{ route('pagamentos-novo.index') }}" class="text-sm text-slate-500 hover:underline">Voltar</a>
    </div>
</div>
